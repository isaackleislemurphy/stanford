...
class LinUCB:
	...
	def choose(self, x):
		...
		for idx_a in range(self.n_arms):
			...
			p_ta = theta_ta.T @ x + self.alpha * np.sqrt(
				x.T @ inv(self.A[idx_a]) @ x
			)
	...
	def update(self, x, a, r):
		...
		self.A[a] = np.add(self.A[a], np.outer(x, x))
		self.b[a] = np.add(self.b[a], r * x)
		#######################################################
		#########          END YOUR CODE.          ############
...
def main(args):
	...
	for k, u in product(args.update_freq, args.update_arms_strat):
		...
		for seed in args.seeds:
			np.random.seed(seed)
			sim = Simulator(num_users=args.num_users, num_arms=args.num_arms, num_features=args.num_features, update_freq=k, update_arms_strategy=u)
...
if __name__ == '__main__':
	...
	parser.add_argument('--update-arms-strat', '-u', type=str, nargs='+', default=['none'], choices=['none', 'counterfactual', 'popular', 'corrective'])
