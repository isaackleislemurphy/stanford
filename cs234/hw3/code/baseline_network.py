...
class BaselineNetwork(nn.Module):
    ...
    def __init__(self, env, config):
        ...
        self.network = build_mlp(
            ...
            output_size=1, # recall output dim of baseline
            n_layers=config.n_layers,
            size=config.layer_size
        )
        # effectively same as policy_gradient.py
        self.optimizer = Adam(
    ...
    def calculate_advantage(self, returns, observations):
        ...
        # a bit of work after passing through network to get back to numpy
        # need to:
        # * pass through network (get tensor)
        # * tell it not to worry about gradient
        # * kick back to numpy
    ...
    def update_baseline(self, returns, observations):
        ...
        # standard torch practice here
        self.optimizer.zero_grad()
        mse = nn.functional.mse_loss(returns, self.forward(observations))
        mse.backward()
        self.optimizer.step()
        #######################################################
        #########          END YOUR CODE.          ############
