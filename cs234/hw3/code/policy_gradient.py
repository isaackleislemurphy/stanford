...
class PolicyGradient(object):
    ...
    def init_policy(self):
        ...
        # instruction # 2.)
        # categorical policy init() takes network; Gaussian takes network and dims
        self.policy = (
            CategoricalPolicy(network=self.network) if self.discrete
            else GaussianPolicy(network=self.network, action_dim=self.action_dim)
        )
        # instruction # 3.)
        self.optimizer = Adam(
            # self.policy.network.parameters(), # ED: 885
    ...
    def get_returns(self, paths):
        ...
        """
        gamma = self.config.gamma # save temporarily
        # helper to calculate return
        compute_rewards = lambda rvec, gamma: (gamma ** np.array(range(len(rvec)))).T @ rvec
    ...
    def update_policy(self, observations, actions, advantages):
        ...
        # gradients zero'd, per usual
        self.optimizer.zero_grad()
        # doing in two steps so I can debug
        action_dist = self.policy.action_distribution(observations)
        # TODO: verify this
        avg_loss = (
            -advantages * action_dist.log_prob(actions)
        ).mean()
        avg_loss.backward()
        self.optimizer.step()
