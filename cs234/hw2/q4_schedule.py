...
class LinearSchedule(object):
    ...
    def update(self, t):
        ...
        if t < self.nsteps:
            ...
            self.epsilon = np.subtract(
                self.eps_begin, # move down from starting point
                np.multiply(
                    np.divide(t, self.nsteps), # percentage through the interpolation
                    np.subtract(self.eps_begin, self.eps_end) # amount we need to move down
                )
            )
        # otherwise do not do anything to epsilon
        else:
            self.epsilon = self.eps_end

        # final sanity check
        assert self.epsilon >= self.eps_end and self.epsilon <= self.eps_begin
...
class LinearExploration(LinearSchedule):
    ...
    def get_action(self, best_action):
        ...
        # as instructed, do a random action
        if np.random.uniform() < self.epsilon:
            # for me: found via dir(self.env.action_space)
            return self.env.action_space.sample()
        # best action with (1 - epsilon)
        return best_action
        ##############################################################
        ######################## END YOUR CODE #######################
